...
export class IoHistoryEngine {
	...
	public async run(historyDays: number, allStates: AnyState[]): Promise<boolean> {
		...
		this.logf.debug('%-15s %-15s %-10s\n%s', this.constructor.name, 'run()', 'srcStates', JSON.stringify(Object.keys(srcStates), 		null, 4));
		this.logf.debug('%-15s %-15s %-10s\n%s', this.constructor.name, 'run()', 'dstStates', JSON.stringify(Object.keys(dstStates),		null, 4));
		...
		for (const stateId of dstStateIds.filter(dstStateId => ! sqlStateIds.includes(dstStateId))) {
			this.logf.warn('%-15s %-15s %-10s %-50s missing', this.constructor.name, 'run()', 'datapoint', stateId);
		}
		...
		const deletePeriodMs = 1000*3600*24 * IoHistoryEngine.ReadDaysLimit/2;
		for (let deleteFrom = fromTs; deleteFrom <= Date.now(); deleteFrom += deletePeriodMs) {
			...
			const deletedTotal = (affectedRows['ts_number'] ?? 0) + (affectedRows['ts_bool'] ?? 0);
			if (deletedTotal > 0) {
		...
		// replay: hist_init seeds initial values, hist_replay drives hist_write
		await this.hist_init(fromTs, allStates);
		await this.hist_replay(fromTs, srcStates);
		...
		for (const srcState of Object.values(srcStates)) {
			...
			if (state  &&  state.val !== null  &&  state.val !== srcState.val) {
				this.logf.debug('%-15s %-15s %-10s %-50s %s   %s', this.constructor.name, 'run()', 'update', srcState.stateId, dateStr(state.ts), valStr(state.val));
	...
	private async hist_replay(replayFromTs: number, srcStates: Record<string, AnyState>): Promise<void> {
		const srcStateIds = Object.keys(srcStates).sort();

		// adaptive read: start at 25000 rows/day, adjust based on observed density
		const rowsLimit			= 1.5 * this.ReadSize;		// 1.5× to detect a full-chunk hit without re-reading
		let   rowsPerDay		= 25000;					// [rows/day] initial estimate
		let   rowsPeriodDays:	number;
		let   prevBatch:		Promise<void>	= Promise.resolve();
		for (;;) {
			await new Promise((res, _rej) => setTimeout(res, 10));		// yield to avoid starving the event loop between chunks

			// rowsPeriodDays capped at ReadDaysLimit to bound single SQL query size
			rowsPeriodDays		= Math.min(IoHistoryEngine.ReadDaysLimit, this.ReadSize/rowsPerDay);
			const beforeTs		= replayFromTs + 1000*3600*24*rowsPeriodDays;

			this.logf.debug('%-15s %-15s %-10s %-14s %-28s %-6s %s', this.constructor.name, 'hist_replay()', 'reading', sprintf('%.1f days', rowsPeriodDays), `before ${dateStr(beforeTs)}`, 'from', dateStr(replayFromTs));
			const now = Date.now();
			const chunkRows = await this.sql.readHistory(srcStateIds, {
				'from':			replayFromTs,
				'before':		beforeTs,
				'ack':			true,
				'isNull':		false,
			...
			this.logf.debug('%-15s %-15s %-10s %-14s %-28s %-6s %s   (%4.1f s)', this.constructor.name, 'hist_replay()', 'read', `#${String(chunkRows.length)}`, `before ${dateStr(beforeTs)}`, 'from', dateStr(replayFromTs), (Date.now() - now)/1000);

			if (chunkRows.length === 0) {
				if (replayFromTs <= Date.now()) {
					// gap in data: keep advancing at minimum density rather than breaking
					rowsPerDay = this.ReadSize/IoHistoryEngine.ReadDaysLimit;
				} else {
					break;
				}
			}
			...
			const firstTs = chunkRows[0]?.ts;
			const lastTs  = chunkRows.slice(-1)[0]?.ts;
			if (firstTs !== undefined  &&  lastTs !== undefined) {
				rowsPerDay = chunkRows.length * 1000*3600*24/(lastTs - firstTs);
				if (chunkRows.length === rowsLimit) {
					continue;
				}
			}

			// process previous chunk concurrently with current read; no await — intentional pipeline overlap
			await prevBatch;
			prevBatch = this.hist_replayRows(chunkRows, srcStates);		// no await — overlaps with next SQL read

			replayFromTs = beforeTs;
		}
	...
	private async hist_replayRows(chunkRows: SqlHistoryRow[], srcStates: Record<string, AnyState>): Promise<void> {
		for (const row of chunkRows) {
			if		(row.ts < this.histNow)  { this.logf.error('%-15s %-15s %-10s %-50s %s < %s', this.constructor.name, 'hist_replayRows()', 'row', row.id, dateStr(row.ts), dateStr(this.histNow)); throw new Error(''); }
	...
	private async hist_setNow(nextNow: number): Promise<void> {
		...
		while (this.histTimers[0] !== undefined) {
			...
			// re-queue interval timer or remove one-shot timer
	...
	private async hist_flush(): Promise<void> {
		const history		= this.histWriteCache.splice(0, this.histWriteCache.length);		// drain in-place
